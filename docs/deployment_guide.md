...
- **自定义日期时间选择器 Popover（v2.4+ / v2.5 滚轮优化）**：
  - 点击「选择提醒时间」触发区域弹出紧凑浮层，采用两列布局（左列日期/时间，右列循环/操作）
  - **滚轮选择器**（v2.5+）：日期（月/日）和时间（时/分）使用紧凑滚轮组件，支持拖拽/滚轮/点击三种交互
  - **「现在」快捷按钮**（v2.5+）：一键设为当前系统时间
  - 支持日期预设快捷选择：今天 / 明天 / 后天 / 下周一
  - 支持时间预设快捷选择：现在 / 8:00 / 12:00 / 20:00
