...
- **核心功能**：输入、添加、标记完成（自动沉底）、删除、**提醒日期 + 到点提示音**（自定义 MP3 / 合成音兜底）、**自定义日期时间选择器 Popover**（替换原生 datetime-local，支持预设快捷选择 + 手动精调）、**循环提醒**（支持预设间隔 / 自定义间隔 / 完成自动开启下一轮 / 完成次数记录）、**双组收纳**（未完成≥5 / 已完成≥3）、**localStorage 永久持久化**（支持跨会话提醒调度）、DOM 节点复用实现无闪动更新。
...
- **提醒功能（需手动验证，Playwright 不支持音频输出）**：
  - 输入「8:00 吃饭」自动解析时间，日期时间选择器同步显示
  - 点击「🔊 测试音效」按钮有声音 + 按钮脉冲动画
  - 设置 3 秒后提醒 → 到点播放 MP3 / 合成音 + 红框抖动 + 史迪奇气泡
- **自定义日期时间选择器 Popover（v2.4+）**：
  - 点击「选择提醒时间」触发区域弹出紧凑浮层，支持日期/时间预设快捷选择
  - 支持循环提醒开关，预设间隔：每 30 分钟 / 每 1 小时 / 每 1 天 / 每 1 周
  - 支持自定义循环间隔（数值 + 单位）
  - 循环待办勾选完成后自动记录次数并推进到下一个周期
- **悬浮 Tooltip（v2.4+）**：鼠标悬浮带提醒的待办项，显示提醒时间、循环设置、完成次数
...
```
project-root/
├── index.html               # 页面主文件（v2.4+：含自定义日期时间选择器 Popover 结构 + 操作指南/GitHub 入口按钮）
├── user_guide.html          # 【v2.3+】内置操作指南页（v2.4+ 新增 3.1 循环提醒章节，共 10 章节）
├── style.css                # 样式文件（v2.4+：Popover 紧凑化 + 循环徽章 + 完成次数 + z-index 层级修复）
├── script.js                # 交互脚本（v2.4+：datetimePickerModule + 循环提醒调度 + toggleTodo 完成自动开启下一轮）
...
**Q7: 循环提醒的完成自动开启下一轮是怎么工作的？（v2.4+）**
A: 当循环待办被勾选完成时，系统会：
1. 将 `completionCount` 加 1，记录已完成次数
2. 推进 `remindAt` 到下一个周期（当前时间 + 循环间隔）
3. 保持未完成态（`done = false`），这样待办不会沉底消失
4. 自动调度下一轮提醒定时器
5. 桌宠弹出庆祝气泡「完成第 N 次！下一轮提醒已安排」

如需停止循环，点击待办项右侧的 ⏹ 按钮即可。

**Q8: 日期时间选择器的 Popover 为什么有时点不开？**
A: 请确保点击的是触发区域（显示「选择提醒时间（可选）」或已选时间文本的框），而不是 Popover 内部的元素。点击 Popover 外部会关闭浮层（防止遮挡页面），这是正常的交互行为。
